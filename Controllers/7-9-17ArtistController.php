...
class ArtistController extends Controller {
	...
	$this->validate($request,
            ...
            [
				...
				'artist_profession'     => 'Artist Profession is required',
				'total_exp'         	=> 'Total Experiences is required',
			...
			$photo = $request->file('photo');
			 ...
			 $banner = $request->file('banner');
			  if($banner){
				...
				DB::table('users')
				->where('id', $request->id)	
				->update(['admin_type'=>$request->admin_type,'first_name' => $request->firstname,'last_name' => $request->lastname,'phone_no' => $request->phone_no,'dob' => $request->dob,'artist_profession' => $request->artist_profession,'total_exp' => $request->total_exp,'competencies' => $request->competencies,'address' => $request->address,'country' => $request->country,'states' => $request->states,'city' => $request->cities,'postcode' => $request->postcode,'work_ling' => $request->work_ling,'description' => $request->description,]);
...
public function artworks_add()
	...
	public function digital_arts_added(Request $request)
	{
		  ...
		  $this->validate($request,
			...
			return redirect('artist/artworks/view')->with('message','Digital Art added successfully');
	}
	...
	public function photographers_added(Request $request)
	{
		  ...
		  $this->validate($request,
			...
			return redirect('artist/artworks/view')->with('message','Photograph added successfully');
	}
	...
	public function audio_added(Request $request)
	{
		  ...
		  $this->validate($request,
			...
			return redirect('artist/artworks/view')->with('message','Audio added successfully');
	}
	...
	public function video_added(Request $request)
	{
		...
		//'video_url.required'			=> 'Video url is required',	
		  ...
		  $this->validate($request,
			...
			'photo.video'           	=> 'Required|video|mimes:mp4,webm',
...
// ->save(new \FFMpeg\Format\Video\WebM(), $dpath.'/export-webm.webm');
 //->save(new \FFMpeg\Format\Video\X264(), $dpath.'/export-x264.mp4')
			...
			return redirect('artist/artworks/view')->with('message','Video added successfully');
	}
	...
	return redirect('artist/artworks/edit')->with('message','Artwork updated successfully');

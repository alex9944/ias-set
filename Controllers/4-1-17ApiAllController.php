...
$arrays['features'] = MerchantsType::all();			
   ...
   return response()->json($data , $responsecode, $header, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);	
    ...
    {
			$users=LovelywallSubscription::where('user_id',$request->user_id)->first();
		//	dd(DB::getQueryLog());
			if(!$users)
			{
					...
					$user->responsemessage = $request->responsemessage;
					$user->transactionid = $request->transactionid;
					$user->save();
					
if($request->responsemessage=="Transaction Successful"){
